...
class ThresholdController:
    ...
    def __init__(self,real=False, bridge=None,offset=0x0) -> None:
        self.modules = {
            ...
            'gas'            : Threshold('gas',           'float', real,        bridge, offset+0x8*14,  offset+0x4+0x8*14,      0,   100),
            'settemp'       : Threshold('settemp',           'float', real,        bridge, offset+0x8*15,  offset+0x4+0x8*15,      0,   100),
        }
        if(real) :
            logger.debug(" \n\
                    ThresholdController: \n\
                ...
                .format(
                    ...
                    hex(self.modules.get('gas').offset.get('min')),              hex(self.modules.get('gas').offset.get('max')),
                    hex(self.modules.get('settemp').offset.get('min')),              hex(self.modules.get('settemp').offset.get('max')),
                )
            )
